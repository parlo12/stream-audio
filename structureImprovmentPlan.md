...
| **4. Quotas + pause-ahead** ✅ DONE (Jun 15 2026; APNs deferred) | content-service-local `plan_limits` + `usage_events` + go-redis monthly counters; `checkAndConsume` gates uploads + transcribe_pages (per-page, idempotent); structured `429` paywall; pause-ahead window (free books stop N pages past the listener, resumed on playback progress). Verified live (cap + 429). **Deferred:** APNs push (needs Apple `.p8` + iOS push registration), streaming metering. Set real `plan_limits` numbers via SQL. | shipped (notifications still MQTT-only) |
| **5. Polish** ✅ DONE (Jun 15 2026; autoscaling deferred) | **5A:** asynqmon at `/admin/grafana`-style `/admin/queues/` (basic auth) + `stream_pages` streaming metering via `checkAndConsume`. **5B:** content-service `/metrics` (asynq `QueueMetricsCollector` via `metrics.go`) → Prometheus → Grafana dashboard "Narrafied — asynq queues" at `/admin/grafana/` (Grafana pinned to `11.2.0`; `latest` crash-loops on datasource provisioning). **5C:** `hls:package` asynq task (`hls.go` → ffmpeg AAC 10s segments → R2 `audio/{book}/{page}/hls/`), `HLSPath` on chunk, `GET /pages/:page/hls.m3u8` rewrites segment lines to short-TTL presigned R2 URLs; iOS `playPage` HEADs HLS then falls back to per-page MP3 (branch `phase5c-hls-playback`). Verified live end-to-end (playlist + 200 segment). **Deferred:** worker autoscaling (needs orchestrator; manual `--scale`), APNs. **Known follow-up:** cascade book-delete leaves orphaned HLS `.ts` segments in R2 (only the playlist key is DB-tracked) — purge by `audio/{book}/` prefix. | shipped |
